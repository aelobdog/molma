...
@test distinct_types_are_enforced_test :: proc(t: ^testing.T) {
	frac := FracVec3{1, 2, 3}
	cart := CartVec3{1, 2, 3}
	testing.expect_value(t, frac, FracVec3{1, 2, 3})
	testing.expect_value(t, cart, CartVec3{1, 2, 3})
}

@test atom_creation_test :: proc(t: ^testing.T) {
	atom := Atom{position = FracVec3{0.25, 0.5, 0.75}, atomic_number = 6}
	testing.expect_value(t, atom.atomic_number, u16(6))
	testing.expect_value(t, atom.position, FracVec3{0.25, 0.5, 0.75})
}

@test lattice_creation_test :: proc(t: ^testing.T) {
	lat := Lattice{a = CartVec3{5.67, 0, 0}, b = CartVec3{0, 5.67, 0}, c = CartVec3{0, 0, 5.67}}
...
@test molecule_creation_test :: proc(t: ^testing.T) {
	mol := Molecule {
		lattice = Lattice{a = CartVec3{1, 0, 0}, b = CartVec3{0, 1, 0}, c = CartVec3{0, 0, 1}},
	}
	append(&mol.atoms, Atom{position = FracVec3{0, 0, 0}, atomic_number = 1})
	append(&mol.atoms, Atom{position = FracVec3{0.5, 0.5, 0.5}, atomic_number = 8})
